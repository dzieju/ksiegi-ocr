...
class MailSearchUI:
    ...
    def create_search_criteria_widgets(self, save_pdf_callback=None):
        ...
        ttk.Label(self.parent, text="Wyszukaj w pliku PDF (automatyczny zapis):").grid(row=4, column=0, sticky="e", padx=5, pady=5)
        ...
        # Checkboxes
        ttk.Checkbutton(self.parent, text="Tylko nieprzeczytane", variable=self.vars['unread_only']).grid(row=6, column=0, sticky="w", padx=5, pady=5)
        ttk.Checkbutton(self.parent, text="Tylko z załącznikami", variable=self.vars['attachments_required']).grid(row=6, column=1, sticky="w", padx=5, pady=5)
        
        # Attachment filters
        ttk.Label(self.parent, text="Nazwa załącznika (zawiera):").grid(row=7, column=0, sticky="e", padx=5, pady=5)
        ttk.Entry(self.parent, textvariable=self.vars['attachment_name'], width=40).grid(row=7, column=1, padx=5, pady=5)
    ...
    def create_date_period_widgets(self):
        ...
        ttk.Label(self.parent, text="Okres wiadomości:", font=("Arial", 10, "bold")).grid(row=9, column=0, sticky="w", padx=5, pady=(15, 5))
        
        # Create frame for period buttons
        period_frame = ttk.Frame(self.parent)
        period_frame.grid(row=9, column=1, columnspan=2, sticky="w", padx=5, pady=(15, 5))
    ...
    def create_control_widgets(self, search_callback):
        ...
        search_frame.grid(row=10, column=0, columnspan=3, pady=20)
    ...
    def create_results_widget(self):
        ...
        results_frame.grid(row=11, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")
        
        # Configure grid weights for the parent to allow dynamic resizing
        self.parent.grid_rowconfigure(11, weight=1)  # Results row gets all extra space
