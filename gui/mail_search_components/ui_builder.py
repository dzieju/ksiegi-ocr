...
class MailSearchUI:
    ...
    def __init__(self, parent, variables, discover_callback, pdf_select_callback=None):
        self.parent = parent
        self.vars = variables
        self.discover_callback = discover_callback
        self.pdf_select_callback = pdf_select_callback or (lambda: None)
    ...
    def create_search_criteria_widgets(self):
        ...
        pdf_frame.grid(row=4, column=1, columnspan=2, sticky="ew", padx=5, pady=5)
        
        # PDF file path entry (read-only display)
        pdf_path_entry = ttk.Entry(pdf_frame, textvariable=self.vars['pdf_file_path'], width=35, state='readonly')
        pdf_path_entry.pack(side="left", fill="x", expand=True)
        
        # PDF file selection button
        ttk.Button(pdf_frame, text="Wybierz PDF", command=self.pdf_select_callback).pack(side="right", padx=(5, 0))
        
        # PDF search text input
        ttk.Label(self.parent, text="Fraza do wyszukania:").grid(row=5, column=0, sticky="e", padx=5, pady=5)
